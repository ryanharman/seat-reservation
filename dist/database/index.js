...
mongoose_1.default.connection.on("connected", () => {
    logging_1.default.info(NAMESPACE, `Mongoose default connection open to ${dbURI}`);
});
// If the connection throws an error
mongoose_1.default.connection.on("error", (err) => {
    logging_1.default.error(NAMESPACE, `Mongoose default connection error: ${err}`);
});
...
process.on("SIGINT", () => {
    mongoose_1.default.connection.close(() => {
        logging_1.default.info(NAMESPACE, `Mongoose default connection disconnected through app termination`);
        process.exit(0);
    });
});
