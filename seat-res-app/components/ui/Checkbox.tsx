...
const Checkbox = ({ height = "5", width = "5", onChange, initialValue = false }: ICheckboxProps) => {
  ...
  return (
    <label className={`h-${height} w-${width} flex flex-shrink-0 relative transition-all cursor-pointer`}>
      <input
        type="checkbox"
        className="h-full w-full appearance-none cursor-pointer transition-all rounded-md bg-transparent border border-gray-300 hover:border-gray-600 checked:bg-blue-600"
        checked={value}
